...
def analyze_zN(z, outdir, vg, groups, skip_umap=False, num_pcs=2, num_ksamples=20, num_pc_samples=10):
    ...
    if zdim > 2 and not skip_umap:
        ...
        new_marker_size = 8  # Adjust this value as needed
        for handle in handles:
            handle.set_markersize(new_marker_size)
    ...
    analysis.scatter_annotate(pc[:,0], pc[:,1], centers_ind=centers_ind, annotate=True, plot_scatter=True,)
    plt.xlabel('PC1')
    plt.ylabel('PC2')
